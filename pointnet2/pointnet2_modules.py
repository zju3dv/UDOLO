...
from utils.roipool3d.roipool3d_utils import roipool3d_gpu
from models.ap_helper import flip_axis_to_camera_torch, flip_axis_to_depth_torch
...
class PointnetSAModuleVotes(nn.Module):
    ...
    def forward(self, xyz: torch.Tensor,
                ...
                tra_box3d=None,
                only_use_xyz=False) -> (torch.Tensor, torch.Tensor):
        r"""
        Parameters
        ----------
        xyz : torch.Tensor
        ...
        if (tra_box3d is not None) and only_use_xyz:
            tra_xyz = tra_box3d[..., :3]
            tra_xyz[..., 1] = tra_xyz[..., 1] - tra_box3d[..., 3] / 2
            tra_xyz = flip_axis_to_depth_torch(tra_xyz)
            new_xyz = torch.cat([new_xyz, tra_xyz], dim=1)
        ...
        if tra_box3d is not None and (not only_use_xyz):
            features_flip = features.permute(0, 2, 1).contiguous()
            pts = flip_axis_to_camera_torch(xyz)
            pooled_features, pooled_empty_flag = \
                roipool3d_gpu(pts, features_flip, tra_box3d, 0, sampled_pt_num=self.nsample)

            valid_proposal = torch.nonzero(pooled_empty_flag[0] == 0).squeeze(1)
            roi_center = tra_box3d[..., 0:3]
            roi_center[..., 1] = roi_center[..., 1] - tra_box3d[..., 3]
            roi_center = roi_center[:, valid_proposal]
            pooled_features = pooled_features[:, valid_proposal]

            pooled_features[..., 0:3] -= roi_center.unsqueeze(dim=2)
            pooled_features[..., 0:3] = pooled_features[..., 0:3] / self.radius
            pooled_features[..., 0:3] = flip_axis_to_depth_torch(pooled_features[..., 0:3])
            pooled_features = pooled_features.permute(0, 3, 1, 2)

            grouped_features = torch.cat([grouped_features, pooled_features], dim=2)
            tra_xyz = flip_axis_to_depth_torch(roi_center)
            new_xyz = torch.cat([new_xyz, tra_xyz], dim=1)
