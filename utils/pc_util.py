...
def sampling_tsdf(pc, num_sample, return_choices=False):
    """ Input is NxC, output is num_samplexC
    """
    if pc.shape[0] < num_sample:
        choice = np.random.choice(pc.shape[0], num_sample - pc.shape[0], replace=True)
        choice = np.concatenate([np.arange(pc.shape[0]), choice])
    else:
        choice = np.random.choice(pc.shape[0], num_sample, replace=False)
    if return_choices:
        return pc[choice], choice
    else:
        return pc[choice]
...
def random_sampling(pc, num_sample, replace=None, return_choices=False, fix_seed=False):
    """ Input is NxC, output is num_samplexC
    """
    if fix_seed:
        np.random.seed(666)
