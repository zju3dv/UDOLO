...
class ProposalModule(nn.Module):
    ...
    def forward(self, xyz, features, end_points):
        ...
        if self.sampling == 'vote_fps':
            ...
            # This gets us a slightly better coverage of *osbject*
            # votes than vote_fps (which tends to get more cluster votes)
            sample_inds = pointnet2_utils.furthest_point_sample(end_points['seed_xyz'], self.num_proposal)
            if 'integrate' in end_points.keys():
                if end_points['integrate'].tra_box3d is not None:
                    integrate = end_points['integrate']
                    # xyzhwl_ry
                    tra_box3d = integrate.tra_box3d.unsqueeze(0)
                    ratio = self.radius / torch.norm(tra_box3d[..., 3:6], p=2, dim=-1, keepdim=True)
                    shrink_dimention = tra_box3d[..., 3:6] * ratio
                    shrink_y = tra_box3d[..., 3] - shrink_dimention[..., 0]
                    tra_box3d[..., 3:6] = shrink_dimention
                    tra_box3d[..., 1] = tra_box3d[..., 1] - shrink_y / 2
                    xyz, features, _ = self.vote_aggregation(xyz, features, sample_inds, tra_box3d)
                else:
